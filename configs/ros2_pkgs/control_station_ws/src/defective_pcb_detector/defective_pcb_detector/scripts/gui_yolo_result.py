...
from communication_interfaces.srv import ComponentDetection
from communication_interfaces.msg import Componentdata,PcbInfoMsg
...
package_share_directory = get_package_share_directory('defective_pcb_detector')
...
class ComponentPublisherNode(Node):
    ...
    def __init__(self):
        ...
        self.detected_componentdata_publisher = self.create_publisher(PcbInfoMsg, '/defective_pcb_info', 1  )
        ...
        self.startframe_topcamera = False
    ...
    def callback_wristcamera_frame(self, msg:ROSImage):
        ...
        if self.startframe_topcamera == False:
            # print("streaming from wrist camera")
            self.incomming_frame = self.imagebridge.imgmsg_to_cv2(self.wrist_camera_image, desired_encoding='bgr8') 
    ...
    def callback_done_callback(self,future_,component_id):
        ...
        try:
            ...
            msg.component_class = component_class

            msg.defect_loc_x = location_x
            msg.defect_loc_y = location_y
            msg.pcb_img = response_.annotated_frame
            self.detected_componentdata_publisher.publish(msg)
...
class ComponentGUI:
    def __init__(self, ros_node,file_path):
        ...
        # vosk_activation_button = tk.Button(self.root, text="   Voice Input   ", image=self.voskicon, compound=tk.TOP,
        ...
        self.completedicon = ImageTk.PhotoImage(self.photo_resize(os.path.join(self.file_path,"completedicon.jpg")))#
        self.completed_button = tk.Button(self.root, text="Detection Completed", image= self.completedicon, compound=tk.TOP,
                        command=self.completed_button_cb, font=("Helvetica", 12))
        self.completed_button.place(relx=0.4, rely=0.04, anchor='nw')

        self.text_frame = tk.Frame(self.root,width=100, height=50)
        
        # self.text_frame.pack(fill=tk.BOTH, expand=True,padx=20, pady=20)  # Fill the entire window and expand completedicon.jpg
        ...
        self.comingfromcamera = tk.Label(self.root)
        self.comingfromcamera.place(relx=0.01, rely=0.5, anchor='nw')

    def completed_button_cb(self):
        self.text_field.insert(tk.END, "The component detection was confirmed\n")
        self.text_field.insert(tk.END, "Streaming from the top camera for heating process\n")
        self.ros_node.startframe_topcamera = True
    ...
    def vosk_activation_button_cb(self):
        self.ros_node.get_logger().info(f"The cobot is listenning to you")
        self.insert_text_and_scroll("The cobot is listenning to you\n")
        msg_to_be_sent_vosk = String()
        msg_to_be_sent_vosk.data = "Activate the voice transcriber"
    ...
    def create_gui(self):
        ...
        for idx, component_name in COMPONENTS.items():
            # Load image
            image_path = os.path.join(self.file_path,f'{component_name.lower()}.jpg')  
    ...
    def update_incoming_frame(self,camera_frame):
        ...
        new_width, new_height = int(image.width * 1), int(image.height * 1)
    ...
    def gui_periodic_update(self):
        # print("HIIII")
        if self.ros_node.incomming_frame is not None and self.ros_node.incomming_frame_flag:
            self.update_incoming_frame(self.ros_node.incomming_frame)
            self.ros_node.incomming_frame_flag = False
            # print("in sd")
        # rclpy.spin_once(self.rosnode, timeout_sec=0.1)
        self.root.after(1, self.gui_periodic_update)
